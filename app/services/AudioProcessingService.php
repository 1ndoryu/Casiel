    ...
    protected function runWorkflow(int $contentId, int $mediaId, AMQPMessage $msg, callable $handleFailure, array &$filesToDelete): void
    {
        $this->swordApiService->getMediaDetails(
            ...
            function ($mediaDetails) use ($contentId, $mediaId, $msg, $handleFailure, &$filesToDelete) {
                ...
                $this->httpClient->get($fullAudioUrl, [], function ($response) use ($localPath, $contentId, $originalFilename, $msg, $handleFailure, &$filesToDelete) {
                    if ($response->getStatusCode() !== 200) {
                        $handleFailure("Could not download file. Status: " . $response->getStatusCode());
                        return;
                    }
                ...
                }, fn($e) => $handleFailure("Exception during download: " . $e->getMessage()));
            },
            $handleFailure
        );
    }
